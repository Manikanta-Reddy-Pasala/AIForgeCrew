"""Prompt-injection sanitization per DESIGN §8.
...
The older registry-audit helper (`assert_no_network_tools`) lived here while
our custom Hermes driver owned a Python tool registry. With real Hermes
(NousResearch/hermes-agent) handling tool dispatch, the equivalent check
belongs in the Hermes skill manifest — see scripts/install-hermes-adapter.sh.
"""
