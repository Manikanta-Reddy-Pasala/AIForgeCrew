...
from ._cmd_tools import TOOLS as _CMD_JOB_TOOLS
...
TOOLS: dict[str, Callable[[dict, str], dict]] = {
    ...
    "mount_folder": _t_mount_folder,
    "unmount_folder": _t_unmount_folder,
    "save_secret": _t_save_secret,
    ...
    "gitlab_pipelines": _t_gitlab_pipelines,
    "gitlab_pipeline": _t_gitlab_pipeline,
    "gitlab_pipeline_watch": _t_gitlab_pipeline_watch,
    ...
    "mcp": _t_mcp,
    "browse": _t_browse,
    # Visual verification: see the running UI instead of guessing at it.
    "ui_check": _t_ui_check,
    "ui_ask": _t_ui_ask,
    "execute_ipython_cell": _t_ipython,
    "delegate_to_agent": _t_delegate,
    "delegate": _t_delegate,   # alias
    # Checking on a running command instead of waiting it out.
    **_CMD_JOB_TOOLS,
}
...
_READONLY_TOOLS = ("file_read", "read_files", "list_dir", "find", "grep", "memory_lookup",
                   ...
                   "web_fetch", "web_crawl", "workflow_search",
                   ...
                   # GitLab CI reads. Same drift the comment above describes:
                   # docs/TOOLS.md marks all three RO, and Plan mode refused
                   # them anyway because this list is a SEPARATE classification
                   # that a new tool does not join by being read-only.
                   "gitlab_pipelines", "gitlab_pipeline",
...
# File-mutating tools the pre-apply "Review edits" gate (Gap D) holds for human
# Approve/Reject even when policy would auto-allow them. ONE shared list, so
# this gate and the team pipeline's can never disagree again.
...
def _is_mutating(name: str, args: dict) -> bool:
    """True when this tool call actually writes (``editor view`` does not)."""
