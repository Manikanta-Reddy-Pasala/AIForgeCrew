...
- mount_folder {{"path": "/abs/host/folder"}}   (docker sandbox: the user wants you to work in a host folder you cannot see — records it, then TELL them to run `./run.sh --mount /abs/host/folder` on the HOST; it is not visible here until they do, so don't try to read it)
