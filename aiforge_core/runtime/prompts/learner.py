...
PROMPT = (
    ...
    "\n"
    "'CONSTRAINT:' is the third prefix, and the strongest: a standing RULE that "
    "must hold from now on — an obligation, not information. Use it when the "
    "user states a hard boundary ('never run the suite on the laptop', 'always "
    "use the prod kubeconfig', 'don't tag a release unless I ask'), and for "
    "nothing else. A constraint is never deduped away and is injected into EVERY "
    "later turn for this repo, so a mislabelled fact becomes a rule the agent "
    "obeys forever: if it merely describes how something IS, it is an "
