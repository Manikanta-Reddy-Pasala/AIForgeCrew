...
JOB_BUILDER = (
    ...
    "INTEGRATION DATA: when the job needs Jira / Confluence / GitLab / email "
    "data, the script MUST call `aiforge-tool <tool_name> '<json args>'` "
    "(e.g. `aiforge-tool jira_search '{\"jql\": \"...\"}'`) — it uses the "
    "system's CONFIGURED integration (URL + auth from Settings) and prints "
    "JSON. NEVER hand-roll curl against the REST APIs: the script has no "
    "credentials and breaks when the integration config changes. "
    "`aiforge-tool --list` shows the callable tools (reads only).\n"
...
WORKFLOW_BUILDER = (
    ...
    "small helper scripts and pass them as scripts:[{name, content, test}] — "
    "they are saved into the workflow's own scripts/ folder (executable) and "
    "future runs execute them instead of re-deriving the commands. Reference "
    "each script by name in the body's steps. For Jira/Confluence/GitLab/"
    "email data a script MUST call `aiforge-tool <tool> '<json>'` (the "
    "configured integration; reads only) — NEVER raw curl to the REST APIs.\n"
    "HARD TEST GATE (enforced, job-builder parity): learn_workflow "
    "syntax-checks each script AND actually RUNS its `test` command (default: "
    "the script itself with no args) — ANY failure refuses the save and "
    "returns the output; fix and retry. So give every script a fast, "
    "side-effect-free test path (e.g. test: \"bash x.sh --dry-run\") and make "
    "it terminate. `test: \"skip\"` is allowed ONLY for a genuinely prod-only "
    "script — say why in the body. Also dry-run the flow yourself with "
    "run_command first and report the actual EFFECT, not just exit 0.\n"
