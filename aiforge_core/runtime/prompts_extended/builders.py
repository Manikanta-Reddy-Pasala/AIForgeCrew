...
JOB_BUILDER = (
    ...
    "run_command (a --dry-run path, or run it against a throwaway dir) and "
    "show the output. Reasoning: prefer plain deterministic shell over "
    "cleverness; handle edge cases; NEVER a destructive op without a guard.\n"
    "Only AFTER the user approves the dry-run, call "
    "create_job_script(name, cron, script). It writes the script to the local "
    "~/.aiforge/jobs folder and schedules it — do NOT hand-place the script "
    "anywhere else, and do not commit it to the repo.\n"
)
