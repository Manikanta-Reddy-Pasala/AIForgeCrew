"""Vendored from aider-chat 0.86.2 (``aider/repomap.py``), Apache-2.0.

WHY VENDORED: AIForge used exactly one thing from aider-chat - the
tree-sitter + PageRank ranked repo map. Depending on the whole package
dragged in 107 transitive packages (mixpanel/posthog telemetry, audio
libs, openai, tiktoken) and aider's EXACT pins on litellm, urllib3,
pillow, requests and friends, which held known CVEs and forced a block of
``override-dependencies`` in pyproject.toml. This file plus
``_vendor_special.py`` and ``queries/`` are that one feature, and depend
only on diskcache, grep-ast, pygments, tree-sitter,
tree-sitter-language-pack and networkx.

Upstream: https://github.com/Aider-AI/aider (Apache License 2.0).
Local changes: aider.dump / aider.waiting.Spinner / tqdm replaced with
no-op shims, aider.special inlined as _vendor_special, the __main__ block
and get_supported_languages_md() dropped along with the now-unused ``sys``
import. Otherwise byte-for-byte upstream so it can be re-synced.
"""
import colorsys
import math
import os
...
from ._vendor_special import filter_important_files


def dump(*args, **kwargs):  # noqa: D401 - aider's debug printer, neutered
    """No-op stand-in for aider.dump.dump (vendored code calls it in comments)."""
    return None


class _NoSpinner:
    """Stand-in for aider.waiting.Spinner - AIForge renders headless."""
...
def tqdm(iterable, *args, **kwargs):  # noqa: D401 - progress bars are noise here
    """Identity stand-in for tqdm; AIForge runs this off a tty."""
    return iterable
