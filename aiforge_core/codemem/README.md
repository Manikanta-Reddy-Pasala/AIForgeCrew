...
`aiforge_core/index/` + `aiforge_core/memory/code_context.py` stack.
...
```
aiforge-codemem doctor                 # check repomix, neo4j, llm
aiforge-codemem ingest <repo>          # Stages 1+2+3 (full)
aiforge-codemem ingest <repo> --force  # rerun ignoring sha cache
aiforge-codemem stats <repo>           # Repo node summary
aiforge-codemem services <repo>        # list services + file counts
```
...
aiforge-codemem ingest tinyrepo \                # real ingest of fixture
    --path aiforge_core/codemem/tests/L1_repo_node/fixtures/tiny_repo
aiforge-codemem stats tinyrepo                   # confirm 5 commands + runbook ≥500 chars
```
...
L1 README: `aiforge_core/codemem/tests/L1_repo_node/README.md`

### L2 — Service extract

```bash
make test-codemem-L2
aiforge-codemem ingest l2demo \
    --path aiforge_core/codemem/tests/L2_service_extract/fixtures/multi_repo
aiforge-codemem services l2demo
```
...
L2 README: `aiforge_core/codemem/tests/L2_service_extract/README.md`
...
During the cutover (Steps 1–9 of the migration plan), legacy `:Symbol` and `:Chunk` nodes from `aiforge_core/index/graphify_loader.py` stay live. To keep constraints from clashing, codemem's nodes use `_v2`-suffixed labels: `File_v2`, `Symbol_v2`, `Chunk_v2`. After Step 10 (legacy delete), a single migration drops the suffix.
...
```python
from aiforge_core.codemem.api import bundle
...
AIFORGE_NEO4J_URI=bolt://127.0.0.1:7687 .venv/bin/pytest aiforge_core/codemem/tests/ -v
```
