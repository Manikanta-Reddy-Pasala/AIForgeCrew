...
def test_symbol_uniqueness(driver) -> None:
    from aiforge_core.codemem.store import schema
    schema.apply(driver)
    with driver.session() as s:
        rows = list(s.run(
            "SHOW CONSTRAINTS YIELD name, labelsOrTypes, properties, type "
            "WHERE 'Symbol_v2' IN labelsOrTypes "
            "  AND properties = ['repo', 'fqname']"
        ))
    assert len(rows) == 1


def test_symbol_kind_index(driver) -> None:
    from aiforge_core.codemem.store import schema
    schema.apply(driver)
    with driver.session() as s:
        rows = list(s.run(
            "SHOW INDEXES YIELD name, labelsOrTypes "
            "WHERE name = 'codemem_symbol_kind'"
        ))
    assert len(rows) == 1
