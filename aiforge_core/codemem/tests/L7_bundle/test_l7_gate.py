...
from aiforge_core.codemem.ingest import flow, repo_summary as rs
from aiforge_core.codemem.ingest.service_extract import ServiceDraft
from aiforge_core.codemem.query import bundle, translator
    ...
    with patch("aiforge_core.codemem.ingest.flow.pack_repo.pack",
         ...
         patch("aiforge_core.codemem.ingest.flow.repo_summary.summarize",
               return_value=rs.RepoSummary(
                   runbook_md="## Build\n\n```\nmake build\n```\n" + ("r" * 600))), \
         patch("aiforge_core.codemem.ingest.flow.service_extract.extract_services",
               return_value=services), \
         patch("aiforge_core.codemem.ingest.file_summary._call_llm",
               return_value=_FILE_SUMMARY_RESPONSE), \
         patch("aiforge_core.codemem.ingest.embed._embed",
...
def test_l7_bundle_with_translator_grounding(driver) -> None:
    ...
    with patch("aiforge_core.codemem.query.translator._embed_query",
               return_value=_FAKE_VEC), \
         patch("aiforge_core.codemem.query.translator._call_llm",
