...
def rescope_globals(*, limit: int | None = None) -> dict:
    ...
    try:
        rows = [d for d in store.load_all("global") if d.get("type") == "learning"]
    except Exception as exc:  # noqa: BLE001
        log.debug("rescope: load failed: %s", exc)
        return {"demoted": 0, "error": str(exc)}
    ...
    for d in rows:
        ...
        # store._scope_of keys on workspace first, so one would already have
        # been filed under its project. There is no repo to restore here.
        meta["scope"] = UNSCOPED
        meta["demoted_from"] = "global"
        meta["demoted_why"] = why
        try:
            r = store.save_node("learning", d.get("id"), meta, body, reindex=False)
        except Exception as exc:  # noqa: BLE001
...
def relabel_magnet_facts(*, limit: int | None = None,
    ...
    from ._render import _brief_upsert, _fact_body

    # Gather a bounded batch of facts, remembering which brief each came from.
    batch: list[dict] = []
    origin: dict[str, str] = {}
    for key in magnets:
    ...
    # Plan the moves first. `_brief_upsert` takes _WRITE_LOCK itself, and that
    # lock is NOT reentrant — calling it from inside the lock self-deadlocks.
    # So: plan unlocked, upsert unlocked (each call locks for its own write),
    # and take the lock only for the source rewrite.
    by_src: dict[str, list[tuple[dict, str]]] = {}
    for item in batch:
        target = labels.get(item["file"])
        src = origin[item["file"]]
        if not target or target == src:
            continue               # unplaceable, or already where it belongs
    ...
    for src, items in by_src.items():
        ...
        for item, target in items:
            try:
                ...
                continue           # fact stays in the magnet, retried next pass
            landed.append(item)
        if not landed:
            continue
        with _WRITE_LOCK:
            # Re-read under the lock: a concurrent write may have changed the
            # brief since we planned, and dropping facts we never read would
            # lose them.
            keep = _facts_of(src)
            for item in landed:
            ...
            try:
                ...
                moved += len(landed)
            except Exception as exc:  # noqa: BLE001 — the facts are already
                log.debug("relabel: rewrite %s failed: %s", src, exc)  # copied
    if moved:
