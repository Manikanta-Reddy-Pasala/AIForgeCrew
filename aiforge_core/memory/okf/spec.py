...
OKF_RULES = (
    ...
    "- Frontmatter MUST contain a non-empty `type:` field. The node types are: "
    "objective, key_result, learning, session, solution (a completed feature/"
    "fix), repo (the hub CARD for one repository — build/test/run/structure/"
    "deploy/gotchas), script (a reusable shell/python script), task (a small-task "
    "recipe).\n"
    "\n"
    "SCOPE & LAYOUT (this bundle is segregated by scope):\n"
    "- GLOBAL knowledge (holds across all repos — user preferences, cross-cutting "
    "rules) lives under `/global/<type>/`.\n"
    "- PROJECT knowledge (specific to one repo — its structure, build command, "
    "conventions, solutions, scripts, tasks, and its repo card) lives under "
    "`/projects/<repo>/<type>/`.\n"
    "- A node's scope is set by frontmatter: `workspace: <repo>` (or "
    "`scope: repo:<repo>`) → that project; else global. Put repo-specific "
    "knowledge in a project, universal rules in global.\n"
    "\n"
    "IDENTITY & LINKS:\n"
    "- The FILE PATH is the concept's identity (e.g. "
    "/projects/CacheLayer/learnings/L-03.md, /global/learnings/L-07.md). No "
    "external IDs or databases.\n"
    "- Link concepts with Markdown links to build the graph. PREFER absolute "
    "bundle-relative links starting with `/` — e.g. link a solution to its repo "
    "card [CacheLayer](/projects/CacheLayer/repo/R-cachelayer.md). Relative links "
    "(./other.md) are also allowed.\n"
