...
| Own AST code chunker; chonkie kept for docs ONLY | chonkie's CodeChunker needs `tree-sitter-language-pack>=1.x`; the vendored RepoMap's tag queries hold us at `0.13` — unresolvable in one env. Line-window fallback so ingestion never breaks | `packages/aiforge_memory/pyproject.toml` comments; `features/chunk/chonkie_adapter.py` | 2026-07 |
...
| Langfuse mirror is SDK-free (raw REST) | The Python SDK's pins conflict with our own; POST to `/api/public/ingestion` over httpx (already a core dep) needs zero extra installs | `integrations/langfuse_adapter.py` | 2026-07-09 |
...
| Postgres→SQLite migration REMOVED | It had been broken since `_PgChatStore` went away with Postgres support: converge ran `scripts/migrate_to_sqlite.py` as a subprocess, it exited 1 on its first import, and converge retried the identical failure every boot. Reviving it needs a psycopg dependency this build does not ship, and the upgrade is not needed. converge now leaves an un-migrated `aiforge-postgres` completely alone rather than pretending — removal stays gated on the marker, which is only written when there was nothing to migrate | `aiforge_core/deploy/converge.py`; `scripts/migrate_to_sqlite.py` deleted | 2026-08-31 |
...
| ragas is a dev overlay, never an app dependency | Its langchain pins conflict with our own in one resolution universe; run via `uv run --with 'ragas<0.4' …` | `integrations/ragas_adapter.py` | 2026-07 |
...
| RepoMap vendored in-tree; `aider-chat` dropped as a dependency | We used exactly one thing from aider-chat — `aider.repomap.RepoMap` (tree-sitter tags + PageRank ranked code map). The package brought 107 transitive deps (mixpanel + posthog telemetry, sounddevice/soundfile/pydub, openai, tiktoken, flake8) and EXACT-pinned litellm/urllib3/pillow/requests/gitpython/starlette, holding 2 CRITICAL litellm CVEs and ~60 HIGH Trivy findings; 0.86.2 was its newest release, so there was no upgrade path through it and pyproject carried 13 `override-dependencies` to force past the pins. `repomap.py` + `special.py` + the 236K of `queries/*.scm` are now vendored under Apache-2.0 with no-op shims for `dump`/`Spinner`/`tqdm`; runtime deps shrink to grep-ast, tree-sitter-language-pack, networkx, diskcache, pygments. Lock lost 26 packages. Output is byte-identical — same code, same tag queries | `aiforge_core/indexing/repomap/`; `NOTICE`; `pyproject.toml` override block | 2026-08-31 |
