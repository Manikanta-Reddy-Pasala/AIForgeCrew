# ONE-52 — Stock transfer non-atomic warehouse+qty multi-step update

**Written by**: Software Architect (Claude Code)
**Date**: 2026-04-20
**Ticket ID**: ONE-52
**Branch**: `aiforge/ONE-52` (created by Architect in MongoDbService)
...
**Sr Developer**: read this file, `rag` relevant idioms, then write `docs/breakdowns/ONE-52.md` with numbered sub-tasks (expect 2-3 sub-tasks: the code change, the test, possibly a cleanup). Commit breakdown on branch `aiforge/ONE-52` in MongoDbService repo. Post ticket comment ending `READY_FOR_DEV`.

**Developer**: after breakdown exists, check out `aiforge/ONE-52`, implement each sub-task with its unit test, commit per sub-task, push, open PR. Final comment `READY_FOR_REVIEW`.
