...
def container_name(exe: str) -> str:
    """The sandbox container, whoever started it.

    `run.sh` may name it something else, and `aiforge box logs` is the command
    every error message points at — it must not fail because the name moved.
    """
    r = subprocess.run([exe, "inspect", "-f", "{{.State.Status}}", CONTAINER],
    ...
    found = [n for n in r.stdout.split() if n]
    return found[0] if found else CONTAINER


def container_state(exe: str) -> str:
    """``running`` / ``exited`` / ``missing`` — the box's own word for it."""
    r = subprocess.run([exe, "inspect", "-f", "{{.State.Status}}", container_name(exe)],
                       capture_output=True, text=True)
...
def render_compose(cfg: Config, mount_paths: list[str], *,
    ...
    # Inside the box the mount list is read as a PATH list, so it is joined
    # with the platform's own separator: a colon would collide with the drive
    # letter on Windows (C:\a:D:\b cannot be split).
    joiner = ";" if platform_is_windows(plat) else ":"
    lines = [
        ...
        f"      - {_yaml('AIFORGE_MOUNTS=' + joiner.join([_box_config_dir(box_home), *[paths.to_box(m, platform=plat) for m in mount_paths]]))}",
    ]
...
def shell(cfg: Config) -> int:
    ...
    exe = require_docker()
    return subprocess.call([exe, "exec", "-it", container_name(exe), "bash", "-l"])
