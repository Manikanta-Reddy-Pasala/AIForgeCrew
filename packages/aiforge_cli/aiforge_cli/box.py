...
def compose_path(cfg: Config) -> Path:
    return cfg.config_dir / ".sandbox" / "cli-compose.yml"
...
def render_compose(cfg: Config, mount_paths: list[str], *,
    ...
    lines = [
        ...
        "    environment:",
        f'      - AIFORGE_RUN_ARGS=--host 0.0.0.0 --port {cfg.port}',
        f'      - AIFORGE_MOUNTS={":".join([str(cfg.config_dir), *mount_paths])}',
    ]
    ...
    lines += [
        "    volumes:",
        f'      - "{cfg.config_dir}:{box_home}/.aiforge"',
    ]
    for m in mount_paths:
        lines.append(f'      - "{m}:{paths.to_box(m, platform=plat)}"')
    lines += [
...
def shell(cfg: Config) -> int:
    ...
    exe = require_docker()
    return subprocess.call([exe, "exec", "-it", CONTAINER, "bash", "-l"])
