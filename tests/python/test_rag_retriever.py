"""Unit tests for aiforge_core.rag.retriever — Phase 2 RAG path.
...
def _stub_retrievers(vec_map: dict, bm25_map: dict):
    ...
    with (
        patch("aiforge_core.rag.retriever._vector_retrieve", side_effect=fake_vec),
        patch("aiforge_core.rag.retriever._bm25_retrieve", side_effect=fake_bm25),
        patch("aiforge_core.rag.retriever._rerank", side_effect=fake_rerank),
        patch("aiforge_core.rag.retriever._get_store", return_value=object()),
    ):
        yield
...
class TestRolePolicy:
    def test_role_policy_applies_correct_tiers(self) -> None:
        ...
        with (
            patch("aiforge_core.rag.retriever._vector_retrieve", side_effect=track_vec),
            patch("aiforge_core.rag.retriever._bm25_retrieve", return_value=[]),
            patch("aiforge_core.rag.retriever._rerank", side_effect=lambda q, h, keep: h[:keep]),
            patch("aiforge_core.rag.retriever._get_store", return_value=object()),
        ):
