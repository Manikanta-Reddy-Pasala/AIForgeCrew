...
from aiforge_core.orchestrator.run_ticket import (
...
def test_doer_git_apply_dirty_worktree_refuses(tmp_path) -> None:
    from aiforge_core.agents.doer import _git_apply_diff
    import subprocess
...
def test_extract_urls_finds_http_https() -> None:
    from aiforge_core.orchestrator import web_fetch as wf
...
def test_doer_calls_llm_and_runs_detectors(tmp_path) -> None:
    ...
    d = registry.build("doer")
    with patch(
        "aiforge_core.orchestrator.llm_client.call_text",
...
def test_tester_returns_test_specs() -> None:
    ...
    with patch(
        "aiforge_core.orchestrator.llm_client.call_json",
        return_value=fake,
    ):
...
def test_learner_writes_episodic_and_procedural() -> None:
    ...
    with patch(
        "aiforge_core.memory.online_learner.record_episodic",
    ) as rec_e, patch(
        "aiforge_core.memory.online_learner.update_procedural",
    ) as upd_p:
        out = L.run(ctx={
            "ticket_id": "TKT-X",
            "repo": "r",
