...
def test_crash_is_caught_and_retried(tmp_path, monkeypatch):
    ...
    assert r["done"] == 1
    assert "c.txt" in set(os.listdir(repo))
...
def test_integration_test_pass_and_fail(tmp_path):
    ...
    assert r["ok"]
    assert r["integration"]["ok"] is True
    assert "integration green" in r["review"]
    ...
    assert r2["integration"]["ok"] is False
...
def test_make_worktree_run_unique(tmp_path):
    ...
    assert wt1 != wt2
    assert b1 != b2
    assert "tok11111" in wt1
    assert "tok11111" in b1
    assert "tok22222" in wt2
    assert "tok22222" in b2
...
def test_run_parallel_uses_run_unique_worktrees(tmp_path):
    ...
    assert seen
    assert not any(n.startswith("sub-") for n in seen)
