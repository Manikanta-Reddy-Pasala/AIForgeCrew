...
def test_graph_traversal(graph):
    ...
    assert "L-01" in learn and "L-02" in learn and "L-03" not in learn  # global + scoped only
...
def test_active_pointer_and_retrieve(graph):
    ...
    assert ctx["objective"]["id"] == "O-01" and "Backtest momentum" in ctx["objective"]["context"]
    assert ctx["active_kr"]["id"] == "KR-01" and "no bias" in ctx["active_kr"]["body"]
...
def test_compile_prompt_shape(graph):
    ...
    assert "<OBJECTIVE id=\"O-01\">" in block and "Stock engine" in block
    assert "<ACTIVE_TASK>" in block and "KR-01 · Backtest logic [in-progress]" in block
    assert "<CRITICAL_RULES>" in block and "non-root" in block
    assert "<RECENT_ACTIVITY>" in block and "ran B" in block
