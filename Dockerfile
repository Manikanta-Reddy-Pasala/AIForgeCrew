...
# OS packages only. python3.12 is Ubuntu 24.04's own; -venv carries ensurepip
# so run.sh can bootstrap uv. No compiler: every Python dep installs as a wheel.
...
# Your identity inside the box. Ubuntu 24.04 ships a `ubuntu` user at uid 1000;
# it is removed first so the common uid 1000 is free for you.
