...
upsert_agent "Tester" "qa" "QA / Tester" "$EM_ID" "$(jq -nc '{
  ...
  adapterConfig:{model:"qwen3.5-9b-mlx",timeoutSec:900,hermesCommand:"'"$HOME"'/.local/bin/hermes-serial"},
...
upsert_agent "Sr Developer" "engineer" "Senior Developer" "$EM_ID" "$(jq -nc '{
  adapterType:"hermes_local",
  adapterConfig:{model:"qwen3.6-35b-a3b",timeoutSec:900,hermesCommand:"'"$HOME"'/.local/bin/hermes-serial"},
...
upsert_agent "Sr Architect" "cto" "Senior Software Architect" "$EM_ID" "$(jq -nc '{
  adapterType:"hermes_local",
  adapterConfig:{model:"gemma-4-26b-a4b-it",timeoutSec:900,hermesCommand:"'"$HOME"'/.local/bin/hermes-serial"},
